...
#'
#' @description
#' Basic computing engine used by \code{\link{lesreg}} to fit a weighted
#' linear expected shortfall regression model.
#'
#' This function works directly with a numeric design matrix, response vector,
#' and observation weights. It does not process formulas, model frames, missing
#' values, subsets, or factor levels. These tasks are handled by the
...
#' @param weights Optional vector of positive observation weights of length
#'   \eqn{n}. If \code{NULL}, independent exponential weights with rate
#'   \code{1} are generated internally. The default is \code{NULL}.
...
#' @param method Method used by \code{\link[quantreg]{rq.wfit}} in the
#'   first-step weighted quantile regression. The default is \code{"br"}.
...
#'   \code{\link[stats]{lm.wfit}} in the second-step weighted least-squares
#'   regression. The default is \code{1e-7}.
#' @param singular.ok Logical. If \code{FALSE}, a singular fit in the
#'   second-step weighted least-squares regression is an error.
#' @param ... Additional arguments passed to \code{\link[quantreg]{rq.wfit}}.
#'
#' @details
#' \code{lesreg.wfit} is the low-level weighted fitting function for the
#' two-step estimator of linear expected shortfall regression.
#'
#' If \code{weights} is \code{NULL}, the function generates observation weights
#' from the exponential distribution with rate \code{1}. These weights are used
#' in both the first-step weighted quantile regression and the second-step
...
#' }
#'
#' In the first step, a weighted linear conditional quantile regression model
#' is fitted by \code{\link[quantreg]{rq.wfit}}:
#' \deqn{
...
#' }
#'
#' In the second step, \code{\link[stats]{lm.wfit}} is used to regress the
#' pseudo-response \eqn{w} on the same design matrix \eqn{X} with the same
#' observation weights. The resulting coefficient vector estimates
#' \eqn{\theta}.
#'
...
#' problem for \eqn{(Y^*,X^*)}. The same weighted lower-tail procedure is then
#' applied to the transformed data. The estimated coefficient vector is
#' reported as \eqn{\theta}, and fitted values are converted back to the
...
#'   \item{beta.hat}{Estimated coefficients from the first-step weighted
#'   conditional quantile regression.}
#'   \item{theta.hat}{Estimated coefficients from the second-step weighted
...
#'   \item{quantile.residuals}{Residuals from the first-step weighted quantile
...
#'   original response scale.}
#'   \item{pseudo.response}{Pseudo-response used in the second-step weighted
#'   least-squares regression, reported on the original response scale.}
#'   \item{pseudo.residuals}{Residuals from the second-step weighted regression
#'   of the pseudo-response on \code{x}.}
...
#'   \item{weights}{Observation weights used in fitting. If \code{weights} is
#'   \code{NULL}, these are the generated exponential weights.}
...
#'   \item{qr}{QR decomposition from the second-step weighted least-squares
#'   regression.}
...
#'
#' ## Weighted fit with user-specified weights
#' weights <- stats::rexp(n, rate = 1)
#' fit.w <- lesreg.wfit(x, y, weights = weights, tau = 0.05)
#' fit.w$beta.hat
#' fit.w$theta.hat
#'
#' ## Weighted fit with internally generated exponential weights
#' fit.exp <- lesreg.wfit(x, y, tau = 0.05)
#' head(fit.exp$weights)
#'
#' ## Upper-tail weighted fit
#' fit.upper <- lesreg.wfit(x, y, weights = weights, tau = 0.05,
#'                          upperTail = TRUE)
#' fit.upper$tail
